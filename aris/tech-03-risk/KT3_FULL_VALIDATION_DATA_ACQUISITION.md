# KT3 Full Validation Data Acquisition and Readiness

## Purpose

本文档记录 KT3 帖子级统一检测模型 `MV-PostGuard` 进行全量验证所需的数据、论文、开源仓库、当前本地状态和下一步动作。它的作用是防止把“论文/代码仓库已找到”误写成“数据集已可全量验证”。

当前权威机器可读入口：

- Registry: `G:\CISCN\CogGuard\system\backend\app\core\risk\config\kt3_post_benchmarks.json`
- Audit script: `G:\CISCN\CogGuard\system\backend\scripts\audit_kt3_post_datasets.py`
- Latest audit report: `G:\CISCN\.tmp\kt3_post_dataset_audit_v2.json`
- Full-validation gate script: `G:\CISCN\CogGuard\system\backend\scripts\summarize_kt3_full_validation.py`
- Latest full-validation gate report: `G:\CISCN\.tmp\kt3_full_validation_gate_report.json`

重跑审计：

```powershell
cd G:\CISCN\CogGuard\system\backend
python scripts\audit_kt3_post_datasets.py `
...
```powershell
cd G:\CISCN\CogGuard\system\backend
python scripts\summarize_kt3_full_validation.py `
  --audit G:\CISCN\.tmp\kt3_post_dataset_audit_v2.json `
  --manifest G:\CISCN\.tmp\kt3_post_cases_full\manifest.json `
  --suite G:\CISCN\.tmp\kt3_post_multiview_ablation_full_registry\report.json `
  --output G:\CISCN\.tmp\kt3_full_validation_gate_report.json
```
...
```powershell
cd G:\CISCN\CogGuard\system\backend
python scripts\run_kt3_post_multiview_ablation.py `
  --case-dir G:\CISCN\.tmp\kt3_post_cases_full `
  --output-dir G:\CISCN\.tmp\kt3_post_multiview_ablation_full_registry `
...
当前 full-registry runner 覆盖 15 个 benchmark。`G:\CISCN\.tmp\kt3_post_multiview_ablation_full_registry\report.json` 中 evaluated datasets 为 MultiOFF、HateXplain、PHEME、FakeSV、mcfend；Jigsaw Toxicity、Hateful Memes、MAMI、MMHS150K、RumourEval 2019、MOCHEG、FACTIFY3M、Fakeddit、MuMiN 因缺少 `kt3-post-case-v1` JSONL 被显式 skipped；Twitter15/16 因本地缺 source tweet text，只有 label/tree，无法作为内容检测训练/测试数据。
...
```powershell
cd G:\CISCN\CogGuard\system\backend
python scripts\build_kt3_post_cases.py `
  --dataset-root G:\CISCN\dataset `
...
2. 对每个新数据集新增 converter，把样本转为 `kt3-post-case-v1`。
...
| Purpose | Path |
|---|---|
| Public/gated dataset repos | `G:\CISCN\dataset\kt3_public\<DatasetName>` |
| Downloaded benchmark files | `G:\CISCN\dataset\<DatasetName>` or `G:\CISCN\dataset\kt3_public\<DatasetName>` |
| Unified cases | `G:\CISCN\.tmp\kt3_post_cases_full` |
| Audit reports | `G:\CISCN\.tmp\kt3_post_dataset_audit_v2.json` |
| Suite reports | `G:\CISCN\.tmp\kt3_post_multiview_ablation_*` |

Do not create KT3 temporary dataset folders under `C:\`.
