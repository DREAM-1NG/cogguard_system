# Risk Review 帖子级统一检测模型与全量验证协议

## 1. 目标

本文档聚焦 Risk Review 帖子级 harmfulness detection。目标是基于高水平文献设计一个统一的帖子级检测模型，覆盖 `tweet / meme / img / video` 四类帖子视图，并定义可在公开数据集和项目数据集上执行的全量验证协议。
...
这些文献支撑 Risk Review 采用“多视图检测器 + 可解释融合”的模型，而不是把所有字段拼成一个手工特征表。

### 2.1 高水平文献到 Risk Review 方法模块的映射

| Risk Review 模块 | 主要参考 | 可迁移的方法思想 | 在 `MV-PostGuard` 中的落点 |
|---|---|---|---|
...
| Agent reviewer / decision optimizer | [MARO, EMNLP 2025](https://aclanthology.org/2025.emnlp-main.291/) | 多专家 Agent 可以做跨域分析、question-reflection 和 decision rule optimization，但不应替代底层可训练检测器 | Risk Review Agent 层用于复核冲突、优化融合规则、回流难例和生成反制叙事；底层判断仍来自四视图检测器与校准融合 |

### 2.2 文献到方法论的进一步归纳

这些工作给 Risk Review 的统一方法论提供了四条约束，而不只是“参考数据集”：
...
2. HateXplain 说明文本 harmfulness 不能只做二分类，还要保留 target、rationale 和可审计证据。Risk Review 的 `D_tweet` 因此要输出 `harm_types / evidence / confidence`，供用户级 MIL 聚合。
...
5. MARO 说明 Multi-Agent 的强项在跨域分析、question-reflection 和 decision-rule optimization。Risk Review 应把 Agent 放在 reviewer/rule optimizer/teacher 位置，而不是让 Agent 取代底层四视图 detector。

## 3. 统一模型：MV-PostGuard

本文建议把 Risk Review 帖子级统一模型命名为 `MV-PostGuard`，即 Multi-View Post-level Guard。它由四类视图检测器、claim/evidence 条件模块和融合裁决层组成。
...
这部分使 Risk Review 能覆盖两类 harm：一类是无需外部 claim 即可判断的 hate/harassment/offensive，另一类是必须依赖 claim/evidence 的 misinformation。
...
根据最新 Risk Review 帖子级研判口径，单条 post 不再被视为一串拼接后的手工特征，而是被拆成四个可独立判定的 Post View：
...
| 数据集 | 视图 | 任务 | 指标 | 当前本地状态 |
|---|---|---|---|---|
| HateXplain | tweet/text | harmfulness、target、rationale | macro-F1、rationale F1、target F1 | ready：本地 `G:\CISCN\dataset\review_public\HateXplain`，已转换 20148 条 |
...
2. `Schema Normalization`：将所有 benchmark 转成统一 `review_post_case` schema。
...
`G:\CISCN\CogGuard\system\backend\app\core\risk\config\review_post_benchmarks.json`
...
```powershell
python G:\CISCN\CogGuard\system\backend\scripts\audit_review_post_datasets.py `
...
| 获取动作 | P0 数据集 | 链接 |
|---|---|---|
| local conversion / validation | HateXplain | [HateXplain](https://github.com/hate-alert/HateXplain)，本地路径 `G:\CISCN\dataset\review_public\HateXplain` |
| local conversion / validation | FakeSV | [FakeSV](https://github.com/ICTMCG/FakeSV)，本地路径 `G:\CISCN\dataset\review_public\FakeSV`；公开部分已可做 video-id/keyword proxy，raw video/features 仍需协议或 HuggingFace 外部资源 |
...
`G:\CISCN\.tmp\review_post_dataset_audit_v2.json`
...
```powershell
python G:\CISCN\CogGuard\system\backend\scripts\build_review_post_cases.py `
  --dataset-root G:\CISCN\dataset `
  --output-dir G:\CISCN\.tmp\review_post_cases_full
```

转换输出采用 `review-post-case-v1` JSONL schema，字段包括 `case_id / dataset / split / source_id / text / labels / views / claim_context / metadata`。该 schema 是后续训练和评测 `MV-PostGuard` 的统一输入，不代表模型已经训练。

当前已完成本地全量转换：

| 数据集 | 输出 | case 数 | 状态 | 说明 |
|---|---|---:|---|---|
| MultiOFF | `G:\CISCN\.tmp\review_post_cases_full\MultiOFF.jsonl` | 743 | partial | 740 条有图文视图，3 条缺图片引用 |
| HateXplain | `G:\CISCN\.tmp\review_post_cases_full\HateXplain.jsonl` | 20148 | converted | official train/val/test split，包含 text harm、target、rationale 监督；unassigned 样本不进入 suite 训练/测试 |
| PHEME | `G:\CISCN\.tmp\review_post_cases_full\PHEME.jsonl` | 6425 | converted | tweet 正文、rumour/veracity、claim category 已转入统一格式 |
| MCFEND | `G:\CISCN\.tmp\review_post_cases_full\mcfend.jsonl` | 23974 | converted | 中文 fake news title/content、label、claim context 已转入统一格式 |
| Twitter15/16 | `G:\CISCN\.tmp\review_post_cases_full\Twitter15_16_dataset.jsonl` | 2308 | partial | 本地副本缺 `source_tweets.txt`，只有 label/tree，不能直接用于文本模型训练 |

manifest 位置：

`G:\CISCN\.tmp\review_post_cases_full\manifest.json`
...
```powershell
python G:\CISCN\CogGuard\system\backend\scripts\run_review_post_text_baseline.py `
  --case-dir G:\CISCN\.tmp\review_post_cases_full `
  --output-dir G:\CISCN\.tmp\review_post_text_baseline
```

该脚本使用 `TF-IDF(char n-gram) + Logistic Regression`，只读取 `review-post-case-v1.text` 和 `labels.harmfulness`，因此是 `tweet/text-only` 对照实验，不是完整的 `MV-PostGuard` 多视图模型。
...
`G:\CISCN\.tmp\review_post_text_baseline\report.json`

预测输出：

- `G:\CISCN\.tmp\review_post_text_baseline\MultiOFF_predictions.jsonl`
- `G:\CISCN\.tmp\review_post_text_baseline\PHEME_predictions.jsonl`
- `G:\CISCN\.tmp\review_post_text_baseline\mcfend_predictions.jsonl`
...
```powershell
python G:\CISCN\CogGuard\system\backend\scripts\run_review_multioff_multiview_ablation.py `
  --case-dir G:\CISCN\.tmp\review_post_cases_full `
  --output-dir G:\CISCN\.tmp\review_multioff_multiview_ablation_hf_safe `
...
`G:\CISCN\.tmp\review_multioff_multiview_ablation_hf_safe\report.json`

预测与缓存输出：

- `G:\CISCN\.tmp\review_multioff_multiview_ablation_hf_safe\text_only_predictions.jsonl`
- `G:\CISCN\.tmp\review_multioff_multiview_ablation_hf_safe\image_only_predictions.jsonl`
- `G:\CISCN\.tmp\review_multioff_multiview_ablation_hf_safe\late_fusion_predictions.jsonl`
- `G:\CISCN\.tmp\review_multioff_multiview_ablation_hf_safe\clip_image_embeddings_24a2abc8bb876cdc.npz`
...
该结果是 Risk Review 帖子级验证的重要进展：第一次在本地公开图文 benchmark 上跑通了 `D_tweet / D_img / late_fusion` 的可复现对照。但它仍不是全量验证，因为只覆盖 MultiOFF、只覆盖 meme/img/text，不覆盖真实 video encoder、claim-conditioned evidence、Jigsaw/Hateful Memes/MAMI/MOCHEG 等 P0 数据集。
...
```powershell
python G:\CISCN\CogGuard\system\backend\scripts\run_review_post_multiview_ablation.py `
  --datasets MultiOFF HateXplain PHEME mcfend FakeSV `
  --case-dir G:\CISCN\.tmp\review_post_cases_full `
  --output-dir G:\CISCN\.tmp\review_post_multiview_ablation_with_fakesv `
  --hf-endpoint https://huggingface.co
```

该脚本把本地可用的 `review-post-case-v1` 数据集放到同一 report 中评估，并对缺失视图显式 skipped。当前权威报告位置已经统一为：

`G:\CISCN\.tmp\review_post_multiview_ablation_full_registry\report.json`
...
- `G:\CISCN\.tmp\review_post_multiview_ablation_full_registry\MultiOFF\text_only_predictions.jsonl`
- `G:\CISCN\.tmp\review_post_multiview_ablation_full_registry\MultiOFF\image_only_predictions.jsonl`
- `G:\CISCN\.tmp\review_post_multiview_ablation_full_registry\MultiOFF\majority_vote_predictions.jsonl`
- `G:\CISCN\.tmp\review_post_multiview_ablation_full_registry\MultiOFF\late_fusion_predictions.jsonl`
- `G:\CISCN\.tmp\review_post_multiview_ablation_full_registry\MultiOFF\learned_fusion_predictions.jsonl`
- `G:\CISCN\.tmp\review_post_multiview_ablation_full_registry\HateXplain\text_only_predictions.jsonl`
- `G:\CISCN\.tmp\review_post_multiview_ablation_full_registry\PHEME\text_only_predictions.jsonl`
- `G:\CISCN\.tmp\review_post_multiview_ablation_full_registry\mcfend\text_only_predictions.jsonl`
- `G:\CISCN\.tmp\review_post_multiview_ablation_full_registry\FakeSV\text_only_predictions.jsonl`
- `G:\CISCN\.tmp\review_post_multiview_ablation_full_registry\FakeSV\video_feature_only_predictions.jsonl`
- `G:\CISCN\.tmp\review_post_multiview_ablation_full_registry\FakeSV\video_text_late_fusion_predictions.jsonl`
...
- `mcfend` 官方方向是中文多源假新闻检测，但当前本地 `review-post-case` 只包含 `news.csv` 中的文本/claim 字段，没有从 `social_context.csv` 或潜在媒体源恢复图像/视频，因此当前只能算 text/claim-context baseline。
