...
| 加入 MINDS / FOREST 统一多尺度 baseline | 完成 | MINDS Christianity 30 epochs；FOREST douban 30 epochs sequence-only |
...
| FOREST | douban | — | 0.3289 | 0.0868 | 30 epochs，best valid MAP@100 epoch=10；sequence-only，训练 loss 为 nan |
...
- FOREST douban 30 epochs CPU sequence-only 长训已完成；重复训练仍耗时约 2.5 小时，且 DeepWalk network 模式因嵌入缺失仍不可用。
