...
function buildPathGraphOption(summary?: DiffusionSummary | null): EChartsOption {
  ...
  const graphData = nodes.map((node) => {
    ...
    return {
      ...
      itemStyle: {
        opacity: objectFocused || isRoot || isKey ? 1 : 0.6,
        borderColor: objectFocused ? '#2563EB' : undefined,
  ...
  const graphLinks = Array.from(mergedEdges.values())
    ...
    .map((edge) => {
      ...
      return {
        ...
        lineStyle: {
          color: objectFocused ? 'rgba(37, 99, 235, 0.92)' : highlighted ? 'rgba(37, 99, 235, 0.72)' : confirmed ? 'rgba(24, 24, 27, 0.62)' : 'rgba(113, 113, 122, 0.3)',
    ...
    series: [
      {
        ...
        right: 24,
        categories: [
          { name: '源头', itemStyle: { color: '#18181B' } },
          { name: '关键节点', itemStyle: { color: '#2563EB' } },
          { name: '普通节点', itemStyle: { color: '#71717A' } },
        ],
        data: graphData,
        links: graphLinks,
        label: {
...
.layer-bar {
  ...
  background: linear-gradient(90deg, #A1A1AA 0%, #18181B 100%);
}
