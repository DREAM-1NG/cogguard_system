"""Review main-system persistence models.

These tables make the MARO-style Review layer auditable without turning the
...
class ReviewProviderConfig(Base):
    """OpenAI-compatible LLM/retrieval provider config with encrypted key."""

    __tablename__ = "review_provider_configs"
...
class ReviewGateDataset(Base):
    """Uploaded Review Gate Dataset manifest and provenance."""

    __tablename__ = "gate_datasets"
    ...
    contract_version: Mapped[str] = mapped_column(String(64), nullable=False, default="review-gate-dataset-v1")
    metadata_json: Mapped[str] = mapped_column(Text, nullable=False, default="{}")
...
class ReviewGateCase(Base):
    """A normalized post/user/community gate case row."""

    __tablename__ = "review_gate_cases"
...
class ReviewGateLabel(Base):
    """Gold label row attached to a gate case."""

    __tablename__ = "review_gate_labels"
...
class ReviewJob(Base):
    """Background job used by Review Agent review/refinement/backfill flows."""
...
class ReviewAgentReportEvidenceRef(Base):
    __tablename__ = "review_agent_report_evidence_refs"
...
class ReviewAgentReportQuery(Base):
    __tablename__ = "review_agent_report_queries"
...
class ReviewAgentReportUncertainty(Base):
    __tablename__ = "review_agent_report_uncertainties"
...
class ReviewAgentReportAction(Base):
    __tablename__ = "review_agent_report_actions"
...
class ReviewAgentDebateTrace(Base):
    __tablename__ = "review_agent_debate_traces"
...
class ReviewAgentFeedback(Base):
    __tablename__ = "review_agent_feedback"
...
class ReviewPolicyAgentWeight(Base):
    __tablename__ = "review_policy_agent_weights"
...
class ReviewPolicyThreshold(Base):
    __tablename__ = "review_policy_thresholds"
...
class ReviewPolicyRule(Base):
    __tablename__ = "review_policy_rules"
...
class ReviewPolicyMetric(Base):
    __tablename__ = "review_policy_metrics"
...
class ReviewPolicyRefinementRound(Base):
    __tablename__ = "review_policy_refinement_rounds"
