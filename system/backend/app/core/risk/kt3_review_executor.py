"""Local KT3 review execution scaffold.

This module moves KT3 one step beyond a planned queue: it executes a deterministic
...
class KT3EvidenceRetriever(Protocol):
    """Optional offline/test retriever hook for KT3 review execution."""
...
class KT3ReviewProvider(Protocol):
    """Optional offline/test reviewer hook for KT3 review execution."""
...
def execute_kt3_review_queue(
    *,
    ...
    evidence_retriever: KT3EvidenceRetriever | None = None,
    review_provider: KT3ReviewProvider | None = None,
) -> dict[str, Any]:
    """Execute local deterministic KT3 review tasks over in-report evidence."""
    post_semantics = post_semantics or {}
    kt3_harmfulness = kt3_harmfulness or {}
    review_queue = review_queue or {}

    corpus = _build_local_corpus(post_semantics, kt3_harmfulness)
    ...
    return {
        "capability_boundary": {
            ...
            "description": (
                "Executes KT3 review tasks with an in-report lexical evidence corpus by default. "
...
def _provider_state(
    evidence_retriever: KT3EvidenceRetriever | None,
    review_provider: KT3ReviewProvider | None,
) -> dict[str, Any]:
    return {
...
def _build_local_corpus(
    ...
    for account in _as_list(_get(kt3_harmfulness, "user_level", "accounts")):
    ...
    for community in _as_list(_get(kt3_harmfulness, "community_level", "communities")):
