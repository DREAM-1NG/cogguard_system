...
This module is intentionally separate from the deterministic Review review queue.
...
from app.core.review.agent_runtime import AGENT_ORDER
from app.core.review.agent_runtime import EXPERT_AGENTS
from app.core.review.agent_runtime import FOLLOWUP_AGENTS
...
from app.core.review.agent_runtime import has_countermeasure_context
...
from app.core.review.propagation_agent import select_propagation_context
...
def _media_inputs_for_post(
    ...
    *,
    ...
    return build_media_inputs_for_post(
        post,
        include_media_base64=include_media_base64,
        max_keyframes=max_keyframes,
    )
...
def _maybe_data_url(uri: str) -> str | None:
    return maybe_data_url(uri)


def _infer_media_type(uri: str) -> str:
    return infer_media_type(uri)
...
def _agent_requires_vision(
    ...
    *,
    ...
    return agent_requires_vision(agent_name, context=context, state=state)


def _vision_input_status(context: dict[str, Any]) -> dict[str, Any]:
    return build_vision_input_status(context)
...
def _provider_input_bundle_for_agent(
    ...
    *,
    ...
    return build_provider_input_bundle_for_agent(agent_name, context=context)


def _provider_should_receive_media(agent_name: str) -> bool:
    return provider_should_receive_media(agent_name)
