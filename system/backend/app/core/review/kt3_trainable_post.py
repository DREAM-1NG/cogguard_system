"""Trainable KT3 post-level building blocks.

This module is intentionally independent from the existing rule/ablation
runtime. It provides small, reproducible PyTorch components for the KT3
    ...
    *,
    ...
    try:  # pragma: no cover - depends on optional local model cache
        ...
        image_bundle = FeatureBundle(
            ...
            capability_boundary="frozen CLIP image encoder features + trainable KT3 adapter",
        )
        text_bundle = FeatureBundle(
            matrix=text_matrix,
            backend="frozen-clip-text",
            model_name=model_name,
            capability_boundary="frozen CLIP text encoder features + trainable KT3 adapter",
        )
    ...
    if backend in {"sentence-transformer", "auto"}:
        try:  # pragma: no cover - optional model availability
            ...
            return FeatureBundle(
                ...
                model_name=model_name,
                capability_boundary="frozen multilingual sentence-transformer features with trainable KT3 heads",
            )
        except Exception:
        ...
        try:  # pragma: no cover - optional model availability
            ...
            return FeatureBundle(
                ...
                capability_boundary=(
                    ...
                    "with trainable KT3 multitask heads"
                ),
            )
        except Exception:
...
def require_torch() -> None:
    if torch is None or nn is None:  # pragma: no cover
        raise RuntimeError("torch is required for KT3 trainable post models")
...
def build_agent_review(detector_outputs: list[dict[str, Any]], fusion: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "schema_version": "kt3-agent-review-v1",
