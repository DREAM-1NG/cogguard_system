"""MARO-style decision rule optimization for Review agent review.
...
def optimize_agent_policy(dataset_manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    if not validation_cases:
        raise ValueError("Review policy optimization requires splits.validation cases")
    ...
    return {
        "schema_version": "review-agent-policy-v1",
        "policy_id": policy_id,
        "created_at": _utc_now(),
        "policy": best["policy"],
        "optimization": {
...
def refine_agent_policy_loop(
    ...
    *,
    ...
    held_out_cases = _as_list(splits.get("held_out") or splits.get("gate") or splits.get("test"))
    if not validation_cases:
        raise ValueError("Review policy refinement requires splits.validation cases")
    if held_out_required and not held_out_cases:
        raise ValueError("Review policy refinement requires a held-out/gate split when held_out_required=true")
    ...
    return {
        "schema_version": "review-agent-policy-refinement-v1",
        "policy_id": policy_id,
        "created_at": _utc_now(),
        "policy": current_policy,
...
def _policy_id(manifest: dict[str, Any], policy: dict[str, Any]) -> str:
    ...
    return f"review-policy-{digest[:12]}"
...
def _refined_policy_id(
    ...
    digest = sha256(json.dumps(payload, ensure_ascii=False, sort_keys=True, default=str).encode("utf-8")).hexdigest()
    return f"review-refined-policy-{digest[:12]}"
