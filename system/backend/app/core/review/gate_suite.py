...
def evaluate_gate_suite(
    *,
    ...
    """Run the available Review layer gates and return one summary report."""
    normalized_dataset = normalize_gate_dataset(gate_dataset)
    ...
    if post_cases:
        gates["post_gate"] = evaluate_post_gate(
            post_cases,
    ...
    if user_gold:
        gates["user_gate"] = evaluate_user_gate(
    ...
    if community_gold:
        gates["community_gate"] = evaluate_community_gate(
            review_harmfulness=review_harmfulness,
    ...
    return {
        "capability_boundary": {
            "status": "implemented_review_layered_gate_suite",
            ...
            "description": (
                "Composes Review Post/User/Community Gates into one offline evaluation "
...
def evaluate_gate_suite_from_dataset(
    *,
    dataset: dict[str, Any],
    review_harmfulness: dict[str, Any] | None = None,
    graph_export: dict[str, Any] | None = None,
) -> dict[str, Any]:
    """Evaluate Review gates from one explicit labelled dataset contract."""
    return evaluate_gate_suite(
