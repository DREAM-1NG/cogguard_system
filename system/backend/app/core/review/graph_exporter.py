"""Review heterogeneous graph export.

This module exports the current Review runtime evidence into a graph-native schema
...
def export_review_heterogeneous_graph(
    *,
    ...
    """Export Review post/account/community evidence as a heterogeneous graph."""
    ...
    for account in accounts:
        ...
        _add_edge(
            ...
            edge_type="member_of",
            attrs={"source": "review_user_level"},
        )
    ...
    return {
        "capability_boundary": {
            ...
            "description": (
                "Exports Review evidence as an account-post-claim-community-target-media "
...
def write_review_graph_artifact(graph: dict[str, Any], artifact_path: str | Path) -> dict[str, Any]:
    """Write a Review graph JSON artifact and return an auditable manifest."""
...
def read_review_graph_artifact(artifact_path: str | Path) -> dict[str, Any]:
    """Read a Review graph JSON artifact and validate it for downstream consumers."""
    path = Path(artifact_path)
    payload = json.loads(path.read_text(encoding="utf-8"))
    if payload.get("schema_version") != GRAPH_ARTIFACT_SCHEMA_VERSION:
        raise ValueError("Unsupported Review graph artifact schema version")
    if payload.get("artifact_type") != "review_heterogeneous_graph":
        raise ValueError("Unsupported Review graph artifact type")
    graph = payload.get("graph")
    if not isinstance(graph, dict):
        raise ValueError("Review graph artifact does not contain a graph object")
    _validate_graph_for_consumer(graph)
    return payload
...
def _validate_graph_for_consumer(graph: dict[str, Any]) -> None:
    if not isinstance(graph.get("schema"), dict):
        raise ValueError("Review graph missing schema")
    if not isinstance(graph.get("summary"), dict):
        raise ValueError("Review graph missing summary")
    if not isinstance(graph.get("nodes"), list):
        raise ValueError("Review graph nodes must be a list")
    if not isinstance(graph.get("edges"), list):
        raise ValueError("Review graph edges must be a list")

    node_ids = set()
    for node in graph["nodes"]:
        if not isinstance(node, dict) or not _text(node.get("id")) or not _text(node.get("type")):
            raise ValueError("Review graph contains an invalid node")
        node_ids.add(node["id"])

    for edge in graph["edges"]:
        if not isinstance(edge, dict) or not _text(edge.get("source")) or not _text(edge.get("target")):
            raise ValueError("Review graph contains an invalid edge")
        if edge["source"] not in node_ids or edge["target"] not in node_ids:
            raise ValueError("Review graph contains a dangling edge")

    if _safe_int(_get(graph, "summary", "node_count")) != len(graph["nodes"]):
        raise ValueError("Review graph node_count does not match nodes")
    if _safe_int(_get(graph, "summary", "edge_count")) != len(graph["edges"]):
        raise ValueError("Review graph edge_count does not match edges")
...
def _add_post_relation_edges(
    ...
    if not post_id:
        return
    for relation_type, target_post_id, source in _post_relation_targets(post):
        if not target_post_id or target_post_id == post_id:
            continue
        _add_node(
            nodes,
            node_id=f"post:{target_post_id}",
            node_type="post",
            attrs={"post_id": target_post_id, "source": "relation_placeholder"},
        )
        _add_edge(
            edges,
            source=f"post:{post_id}",
    ...
    for (community_node_id, target_node_id), attrs in community_targets.items():
        ...
        _add_edge(
            ...
            attrs={
                ...
                "relations": relations,
                "source": "graph_exporter.account_targets",
            },
        )
...
def _quoted_status_id(raw: dict[str, Any]) -> str:
    quoted = raw.get("quoted_status")
    if isinstance(quoted, dict):
        text = _first_text(quoted, ("id_str", "id", "post_id"))
        if text:
            return text
