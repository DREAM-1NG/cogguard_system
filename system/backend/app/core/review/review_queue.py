"""Review reviewer and RAG task queue scaffold.
...
high-risk Review outputs into a structured queue that a future RAG / multi-agent
...
def build_review_queue(
    *,
    ...
    """Build a deterministic review queue from Review post/user/community outputs."""
    ...
    retrieval_tasks = _build_retrieval_tasks(posts, review_harmfulness, max_items=max_items)
    agent_tasks = _build_agent_tasks(review_items, retrieval_tasks, review_harmfulness)
    counter_inputs = _build_counter_narrative_inputs(review_harmfulness, max_items=5)
    ...
    return {
        "capability_boundary": {
            ...
            "description": (
                ...
                "tasks from Review evidence. It does not execute external retrieval "
...
def _build_agent_tasks(
        ...
        tasks.append(
            {
                ...
                "execution_status": "planned_only",
                "requires_external_execution": True,
                "objective": "Retrieve claim, context, and verification evidence for Review items.",
    ...
    if _as_list(_get(review_harmfulness, "global_summary", "claim_rank")):
        tasks.append(
            {
...
def _build_counter_narrative_inputs(
    ...
    *,
    ...
    harm_types = _get(review_harmfulness, "global_summary", "harm_types") or {}
    communities = _as_list(_get(review_harmfulness, "community_level", "communities"))
    top_claims = _as_list(_get(review_harmfulness, "global_summary", "claim_rank"))
