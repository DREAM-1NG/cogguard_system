...
@celery_app.task(name="kt3.execute", bind=True)
def execute_kt3_job(self, job_id: int):
    """Execute a persisted KT3 job and update its DB audit row."""
    return _execute_kt3_job_with_audit(job_id)


def execute_kt3_job_inline(job_id: int, *, startup_delay_seconds: float = 0.0):
    """Run a KT3 job in-process for local deployments without Celery workers."""
    if startup_delay_seconds > 0:
        time.sleep(startup_delay_seconds)
    return _execute_kt3_job_with_audit(job_id, claim_attempts=60, claim_delay_seconds=0.5)


def _execute_kt3_job_with_audit(
    job_id: int,
    *,
    ...
    try:
        ...
        result = _run_async(_execute_kt3_job_async(job_id))
...
async def _execute_kt3_job_async(job_id: int) -> dict[str, Any]:
    ...
    try:
        async with session_factory() as db:
            ...
            if row.job_type == "agent_review":
                ...
                raise ValueError(f"Unsupported KT3 job type: {row.job_type}")
            await db.commit()
            return result
    finally:
...
def _read_job_status(job_id: int) -> str | None:
    ...
    try:
        with engine.connect() as conn:
            result = conn.execute(
                text("SELECT status FROM kt3_jobs WHERE id=:id"),
                {"id": job_id},
            )
            row = result.first()
...
async def _load_job(job_id: int, db: AsyncSession):
    ...
    if row is None:
        raise ValueError(f"KT3 job not found: {job_id}")
    return row
...
async def _execute_agent_review(*, payload: dict[str, Any], user_id: int, db: AsyncSession) -> dict[str, Any]:
    ...
    review_result = await risk_service.run_kt3_agent_review(
...
async def _provider_for_agent_run(db: AsyncSession) -> dict[str, Any]:
    ...
    result = await db.execute(
        select(KT3ProviderConfig)
        .where(KT3ProviderConfig.provider_type.in_(["text_llm", "vision_llm"]))
        .where(KT3ProviderConfig.is_active.is_(True))
        .order_by(KT3ProviderConfig.provider_type.desc())
        .limit(1)
    )
    row = result.scalar_one_or_none()
...
async def _retrieval_provider_for_agent_run(db: AsyncSession) -> dict[str, Any]:
    ...
    result = await db.execute(
        select(KT3ProviderConfig)
        .where(KT3ProviderConfig.provider_type == "retrieval")
        .where(KT3ProviderConfig.is_active.is_(True))
        .order_by(KT3ProviderConfig.id.desc())
        .limit(1)
    )
    row = result.scalar_one_or_none()
    if row is None:
        if getattr(settings, "KT3_RETRIEVAL_API_KEY", "") and getattr(settings, "KT3_RETRIEVAL_BASE_URL", ""):
            return {
                "provider": _build_http_retrieval_provider(
                    base_url=str(settings.KT3_RETRIEVAL_BASE_URL),
                    api_key=str(settings.KT3_RETRIEVAL_API_KEY),
                    search_path=str(getattr(settings, "KT3_RETRIEVAL_SEARCH_PATH", "/search") or "/search"),
                    timeout_seconds=20.0,
                    provider_name=str(getattr(settings, "KT3_RETRIEVAL_PROVIDER_NAME", "") or "env_retrieval"),
                    adapter=str(getattr(settings, "KT3_RETRIEVAL_ADAPTER", "") or ""),
                ),
                "source": "env_fallback",
            }
