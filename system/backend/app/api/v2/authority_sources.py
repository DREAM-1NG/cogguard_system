...
from app.schemas.case_workbench import AuthorityReview, AuthoritySourceAccountCreate, AuthoritySourceCreate
...
def _dump(row):
    data={key:getattr(value,"value",value) for key,value in vars(row).items() if not key.startswith("_")}
    if isinstance(data.get("verification_snapshot"), str):
        import json
        data["verification_snapshot"]=json.loads(data["verification_snapshot"])
    return data
