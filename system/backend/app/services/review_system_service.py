"""Review main-system integration service.

This service owns persistence-oriented transformations for the MARO-style Review
...
def encrypted_provider_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "name": str(payload.get("name") or "Review Provider").strip(),
...
def encrypt_provider_api_key(api_key: str) -> str:
    """Encrypt a provider API key using a Review-specific master key."""
...
def decrypt_provider_api_key(encrypted_value: str | None) -> str:
    ...
    try:
        ...
        raise ValueError("Review provider API key cannot be decrypted with current key") from exc


def provider_public_view(provider: dict[str, Any] | ReviewProviderConfig, *, source: str = "database") -> dict[str, Any]:
...
def normalize_gate_dataset_upload(dataset: dict[str, Any], *, uploaded_by: int = 0) -> dict[str, Any]:
    """Validate and flatten an uploaded Review Gate Dataset contract."""
    validation = validate_gate_dataset_contract(dataset)
    ...
    dataset_row = {
        ...
        "contract_version": validation.get("contract_version") or "review-gate-dataset-v1",
...
def extract_review_backfill_records(report_id: str, report_json: dict[str, Any], *, created_by: int = 0) -> dict[str, Any]:
...
async def create_review_job(
    *,
    ...
    """Create a Review background job and enqueue execution.
    ...
    if job_type not in JOB_TYPES:
        raise ValueError(f"Unsupported Review job type: {job_type}")
    row = ReviewJob(
        job_type=job_type,
        status="pending",
        progress=0,
    ...
    if enqueue:
        try:
            from app.tasks.review_tasks import execute_review_job

            async_result = execute_review_job.delay(row.id)
    ...
    return {
        ...
        "poll_url": f"/api/v1/risk/review/jobs/{row.id}",
...
def _start_inline_job_fallback(job_id: int) -> None:
    ...
    def _runner() -> None:
        try:
            from app.tasks.review_tasks import execute_review_job_inline

            execute_review_job_inline(job_id, startup_delay_seconds=0.5)
        except Exception:
    ...
    thread = threading.Thread(target=_runner, name=f"review-inline-job-{job_id}", daemon=True)
    thread.start()
...
async def activate_provider_config(
    *,
    ...
    if is_active:
        await db.execute(
            update(ReviewProviderConfig)
            .where(ReviewProviderConfig.provider_type == row.provider_type)
...
async def get_gate_dataset_detail(dataset_db_id: int, db: AsyncSession) -> dict[str, Any] | None:
    result = await db.execute(select(ReviewGateDataset).where(ReviewGateDataset.id == dataset_db_id))
...
async def get_review_job(job_id: int, db: AsyncSession) -> dict[str, Any] | None:
    result = await db.execute(select(ReviewJob).where(ReviewJob.id == job_id))
    row = result.scalar_one_or_none()
    if row and row.status in {"pending", "queued"}:
        _start_inline_job_fallback(row.id)
    return review_job_public_view(row) if row else None


async def _get_provider_row(provider_id: int, db: AsyncSession) -> ReviewProviderConfig:
    result = await db.execute(select(ReviewProviderConfig).where(ReviewProviderConfig.id == provider_id))
    row = result.scalar_one_or_none()
    if row is None:
        raise ValueError(f"Review provider not found: {provider_id}")
    return row


def _provider_payload_from_row(row: ReviewProviderConfig) -> dict[str, Any]:
    return {
        "name": row.name,
        "provider_type": row.provider_type,
...
async def list_review_jobs(db: AsyncSession, *, page: int = 1, page_size: int = 20, job_type: str | None = None) -> dict[str, Any]:
    stmt = select(ReviewJob)
    if job_type:
...
def review_job_public_view(row: ReviewJob) -> dict[str, Any]:
...
async def persist_agent_review_rows(
    *,
    ...
    run = rows["run"]
    existing_run = await db.execute(select(ReviewAgentRun).where(ReviewAgentRun.run_id == run["run_id"]))
    ...
    existing_review_ids = {
        item[0]
        for item in (await db.execute(select(ReviewAgentReport.review_id).where(ReviewAgentReport.review_id.in_(review_ids)))).all()
    ...
    for item in rows["reports"]:
        ...
        for evidence in [row for row in rows["evidence_refs"] if row["review_id"] == item["review_id"]]:
            db.add(ReviewAgentReportEvidenceRef(**{k: evidence[k] for k in ("review_id", "doc_id", "source", "title", "url", "evidence_json")}))
        for query in [row for row in rows["queries"] if row["review_id"] == item["review_id"]]:
            db.add(ReviewAgentReportQuery(**query))
        for uncertainty in [row for row in rows["uncertainties"] if row["review_id"] == item["review_id"]]:
            db.add(ReviewAgentReportUncertainty(**uncertainty))
        for action in [row for row in rows["actions"] if row["review_id"] == item["review_id"]]:
            db.add(ReviewAgentReportAction(**action))
        for trace in [row for row in rows["debate_traces"] if row["review_id"] == item["review_id"]]:
            db.add(ReviewAgentDebateTrace(**{k: trace[k] for k in ("run_id", "review_id", "trace_ref", "debate_mode", "trace_json")}))
    await db.flush()
    return {
...
async def persist_feedback_row(
    *,
    ...
    existing = await db.execute(select(ReviewAgentFeedback).where(ReviewAgentFeedback.feedback_id == row_data["feedback_id"]))
...
async def persist_feedback(
    *,
    ...
    feedback_id = str(feedback.get("feedback_id") or f"review-feedback-{uuid4().hex[:12]}")
...
async def feedback_memory_from_db(report_ids: list[str], db: AsyncSession) -> list[dict[str, Any]]:
    stmt = select(ReviewAgentFeedback)
    if report_ids:
        stmt = stmt.where(ReviewAgentFeedback.report_id.in_(report_ids))
    result = await db.execute(stmt)
    rows = []
    for row in result.scalars().all():
        rows.append(
...
async def persist_policy_artifact(
    *,
    ...
    for name, value in (policy.get("agent_weights") or {}).items():
        db.add(ReviewPolicyAgentWeight(policy_id=policy_id, agent_name=str(name), weight=float(value)))
    for name in ("review_threshold", "abstain_threshold", "retrieval_threshold", "countermeasure_threshold"):
        if name in policy:
            db.add(ReviewPolicyThreshold(policy_id=policy_id, threshold_name=name, threshold_value=float(policy[name])))
...
async def activate_policy_in_db(policy_id: str, *, user_id: int, db: AsyncSession) -> dict[str, Any]:
    result = await db.execute(select(ReviewPolicy).where(ReviewPolicy.policy_id == policy_id))
    row = result.scalar_one_or_none()
    if row is None:
    ...
    if not row.can_activate:
        ...
        raise ValueError(f"Review policy cannot be activated: {reasons or 'unknown_reason'}")
    await db.execute(update(ReviewPolicy).where(ReviewPolicy.status == "active_human_approved").values(status="inactive"))
...
async def get_active_policy_artifact(db: AsyncSession) -> dict[str, Any] | None:
    result = await db.execute(select(ReviewPolicy).where(ReviewPolicy.status == "active_human_approved").order_by(desc(ReviewPolicy.activated_at)).limit(1))
    row = result.scalar_one_or_none()
...
async def _policy_artifact_from_row(row: ReviewPolicy, *, db: AsyncSession) -> dict[str, Any]:
    artifact = {
    ...
    rule_result = await db.execute(select(ReviewPolicyRule).where(ReviewPolicyRule.policy_id == row.policy_id).order_by(ReviewPolicyRule.round, ReviewPolicyRule.id))
    artifact["candidate_rules"] = [_json_loads(item.rule_json, {}) for item in rule_result.scalars().all()]
    trace_result = await db.execute(
        select(ReviewPolicyRefinementRound)
        .where(ReviewPolicyRefinementRound.policy_id == row.policy_id)
        .order_by(ReviewPolicyRefinementRound.round, ReviewPolicyRefinementRound.id)
    )
    artifact["refinement_trace"] = [_json_loads(item.trace_json, {}) for item in trace_result.scalars().all()]
    metric_result = await db.execute(
        select(ReviewPolicyMetric)
        .where(ReviewPolicyMetric.policy_id == row.policy_id)
        .order_by(ReviewPolicyMetric.round, ReviewPolicyMetric.split_name, ReviewPolicyMetric.metric_name)
    )
...
def _policy_artifact_summary_from_row(row: ReviewPolicy) -> dict[str, Any]:
    return {
        "policy_id": row.policy_id,
        "activation_status": row.status,
        "policy": _json_loads(row.policy_json, {}),
...
def env_provider_public_view(provider_type: str) -> dict[str, Any] | None:
    if provider_type == "retrieval":
        if not settings.REVIEW_RETRIEVAL_API_KEY or not settings.REVIEW_RETRIEVAL_BASE_URL:
            return None
        return {
            "id": None,
            "name": settings.REVIEW_RETRIEVAL_PROVIDER_NAME or "Environment Retrieval Provider",
            "provider_type": "retrieval",
            "base_url": settings.REVIEW_RETRIEVAL_BASE_URL,
            "model": "",
            "wire_api": "http_json",
            "supports_vision": False,
            "is_active": True,
            "api_key_status": "configured",
            "api_key_masked": _mask_key(settings.REVIEW_RETRIEVAL_API_KEY),
            "source": "env_fallback",
            "metadata": {
                "search_path": settings.REVIEW_RETRIEVAL_SEARCH_PATH,
                "adapter": settings.REVIEW_RETRIEVAL_ADAPTER,
            },
        }
...
def _fernet() -> Fernet:
    secret = str(getattr(settings, "REVIEW_CONFIG_ENCRYPTION_KEY", "") or "").strip()
    if not secret:
        raise ValueError("REVIEW_CONFIG_ENCRYPTION_KEY is required to store provider API keys")
...
def _normalize_provider_type(value: Any) -> str:
    ...
    if provider_type not in PROVIDER_TYPES:
        raise ValueError(f"Unsupported Review provider_type: {provider_type}")
