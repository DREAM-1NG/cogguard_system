...
__all__ = [
    "ALERT_ACTION_STATE", "DEFAULT_MONITOR_INTERVAL_MINUTES", "apply_alert_action", "build_monitor_window",
...
async def apply_alert_action(
    *, alert_id: int, action: str, note: str | None, actor_id: int, db: AsyncSession,
    ...
    db.add(PropagationAlertAction(alert_id=alert.id, action=action, note=str(note).strip() or None, actor_id=actor_id))
    await db.flush()
...
async def run_due_monitor_profiles(db: AsyncSession, *, now: datetime | None = None) -> list[dict[str, Any]]:
    ...
    profiles = (await db.execute(select(PropagationMonitorProfile).where(PropagationMonitorProfile.enabled.is_(True)))).scalars().all()
    results = []
    for profile in profiles:
        previous_success = _as_utc(profile.last_success_at)
        due_at = previous_success + timedelta(minutes=profile.interval_minutes) if previous_success else None
        if due_at is None or due_at <= reference:
            results.append(await run_monitoring_cycle(profile=profile, db=db, now=reference))
    return results
...
async def _create_event_snapshot(
    *, db: AsyncSession, event_id: str, platform: str | None, observed: Mapping[str, Any],
    ...
    fingerprint_source = {
        "event_id": event_id, "platform": _profile_platform(platform), "captured_at": captured_at.isoformat(),
        "observed": summary, "prediction_scope": _as_mapping(prediction.get("data_scope")),
    }
    fingerprint = hashlib.sha256(_json_dumps(fingerprint_source).encode("utf-8")).hexdigest()
    snapshot = EventSnapshotRecord(
        snapshot_id=f"propagation_monitor_{uuid4().hex}", event_id=event_id, data_fingerprint=fingerprint,
        mongo_collection="raw_posts,raw_comments", mongo_key=f"{event_id}:{_profile_platform(platform)}",
        platforms_json=_json_dumps([platform] if platform else []),
        windows_json=_json_dumps({"captured_at": captured_at.isoformat()}),
        quality_json=_json_dumps(_as_mapping(observed.get("data_scope"))),
        provenance_json=_json_dumps({"source": "propagation_monitoring_cycle.v1"}), created_by=0,
    )
    db.add(snapshot)
    await db.flush()
    return snapshot
    ...
    *, db: AsyncSession, event_id: str, platform: str, signals: list[Mapping[str, Any]], severity: str | None,
    ...
    for signal in signals:
        ...
        statement = select(PropagationAlert).where(
            ...
            PropagationAlert.state.in_(OPEN_ALERT_STATES),
        ).order_by(PropagationAlert.last_triggered_at.desc())
        existing = next((item for item in (await db.execute(statement)).scalars().all() if is_deduplicated_alert(
            item, event_id=event_id, platform=platform, alert_type=alert_type, now=now
        )), None)
        item_evidence = {**_as_mapping(evidence), "trigger_signal": _as_mapping(signal)}
        if existing is None:
            existing = PropagationAlert(
                event_id=event_id, platform=platform, alert_type=alert_type, severity=severity, state="new",
                dedupe_key=f"{event_id}:{platform}:{alert_type}", trigger_count=1, first_triggered_at=now,
                last_triggered_at=now, snapshot_id=snapshot_id, model_version_id=model_version_id,
                evidence_json=_json_dumps(item_evidence),
            )
            db.add(existing)
        else:
