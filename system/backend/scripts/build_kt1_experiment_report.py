...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build the final KT1 experiment report from full result artifacts.")
    parser.add_argument("--system-report-dir", default=str(KT1_ROOT / "kt1_system_report"))
    parser.add_argument("--detect-ablation-dir", default=str(KT1_ROOT / "kt1_detect_ablation"))
...
def main() -> None:
    ...
    system_summary = json.loads((system_dir / "kt1_system_summary.json").read_text(encoding="utf-8"))
    ...
    detect_mainline_report.to_csv(output_dir / "kt1_detect_mainline_table.csv", index=False)
    detect_ablation = pd.read_csv(ablation_dir / "kt1_detect_ablation_mean_std.csv") if (ablation_dir / "kt1_detect_ablation_mean_std.csv").exists() else pd.DataFrame()

    report_lines = [
        "# KT1 Full Experiment Report",
        "",
        "## Dataset Scale",
    ...
    if not detect_ablation.empty:
        ...
        ablation_overall.to_csv(output_dir / "kt1_detect_ablation_overall.csv", index=False)
        ablation_overall.to_csv(output_dir / "kt1_detect_ablation_auc_maxf1.csv", index=False)
        report_lines.extend(
            [
                "",
    ...
    report_lines.extend(
        [
            ...
            "## Figures",
            "",
            "- `kt1_detect_main_metrics_heatmap.png`",
            "- `kt1_detect_ablation_auc_maxf1_combined.png`",
        ]
    )

    (output_dir / "kt1_full_experiment_report.md").write_text("\n".join(report_lines) + "\n", encoding="utf-8")
