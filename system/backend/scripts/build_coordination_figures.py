...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build CoordinationDiscover experiment figures.")
...
def main() -> None:
    ...
    if args.detect_baseline_csv:
        ...
        detect_mainline = baseline_df[
            (baseline_df["discover_encoder"] == "magnn_legacy")
    ...
    _plot_detect_main_metrics_heatmap(
        detect_mainline,
        output_dir / "coordination_discover_detect_main_metrics_heatmap.png",
        title="CoordinationDiscover Detect Main Experiment Metrics (Supervised)",
    )
    ...
    _plot_ablation_heatmap(
        focus,
        output_dir / "coordination_discover_detect_ablation_macro_f1_heatmap.png",
        metric="macro_f1_mean",
        title="CoordinationDiscover Detect Ablation Max-F1 Heatmap",
    )
    _plot_ablation_heatmap(
        focus,
        output_dir / "coordination_discover_detect_ablation_auc_heatmap.png",
        metric="auc_mean",
        title="CoordinationDiscover Detect Ablation AUC Heatmap",
    )
    _plot_ablation_dual_heatmap(
        focus,
        output_dir / "coordination_discover_detect_ablation_auc_maxf1_combined.png",
        left_metric="auc_mean",
        left_title="AUC",
        right_metric="macro_f1_mean",
        right_title="Max-F1",
        title="CoordinationDiscover Detect Ablation (Supervised)",
    )
