"""Audit local Review post-level benchmark readiness.
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        default=r"G:\CISCN\.tmp\review_post_dataset_audit.json",
        help="Path to write the JSON audit report.",
    )
    parser.add_argument(
        "--registry",
        default=str(DEFAULT_REGISTRY),
        help="Machine-readable Review post benchmark registry JSON.",
    )
    ...
    report = {
        "schema": "review-post-dataset-audit-v2",
...
def audit_registry_benchmarks(
    ...
    for benchmark in registry.get("benchmarks", []):
        ...
        candidates = [
            ...
            for candidate in (dataset_root / str(alias), dataset_root / "review_public" / str(alias))
        ]
