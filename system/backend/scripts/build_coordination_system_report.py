...
DEFAULT_DETECT_MERGED = CoordinationDiscover_ROOT / "accept_detect_fusion_merged_final" / "detect_metrics_mean_std.csv"
DEFAULT_DISCOVER_MEAN = CoordinationDiscover_ROOT / "accept_discover_magnn_legacy_vs_core_6d_s5_ep20" / "discover_metrics_mean_std.csv"
DEFAULT_ARCHIVE_MANIFEST = CoordinationDiscover_ROOT / "archive_coordination_discover_final_20260628" / "archive_manifest.json"
DEFAULT_REAL_RUN_DIR = BACKEND_ROOT.parent / "output" / "coordination_runs" / "russia"
MAINLINE_DISCOVER_ENCODER = "magnn_legacy"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build CoordinationDiscover system integration report, comparison tables, and heatmaps.")
...
def main() -> None:
    ...
    (output_dir / "coordination_discover_system_summary.json").write_text(
        json.dumps(system_summary, ensure_ascii=False, indent=2),
        encoding="utf-8",
    )
    _write_csv(output_dir / "coordination_discover_detect_mainline_table.csv", detect_summary["mainline_rows"])
    _write_csv(output_dir / "coordination_discover_detect_backend_comparison.csv", detect_summary["backend_rows"])
    _write_csv(output_dir / "coordination_discover_discover_encoder_comparison.csv", discover_summary["encoder_rows"])
    _write_csv(output_dir / "coordination_discover_discover_detect_claim_rows.csv", detect_summary["claim_rows"])

    _plot_detect_heatmap(
        detect_df,
        output_dir / "coordination_discover_detect_auc_heatmap.png",
        metric="auc_mean",
        title="CoordinationDiscover Detect AUC Heatmap (magnn_legacy + sbert)",
    )
    _plot_detect_heatmap(
        detect_df,
        output_dir / "coordination_discover_detect_macro_f1_heatmap.png",
        metric="macro_f1_mean",
        title="CoordinationDiscover Detect Macro-F1 Heatmap (magnn_legacy + sbert)",
    )
    _plot_discover_bar(
        discover_df,
        output_dir / "coordination_discover_discover_modularity_bar.png",
        metric="modularity_mean",
        title="CoordinationDiscover Discover Modularity Comparison",
    )

    report = _build_markdown_report(system_summary, detect_summary, discover_summary)
    (output_dir / "coordination_discover_system_report.md").write_text(report, encoding="utf-8")
...
def _detect_summary_tables(df: pd.DataFrame) -> dict[str, list[dict[str, object]]]:
    mainline = df[
        (df["discover_encoder"] == MAINLINE_DISCOVER_ENCODER)
    ...
    subset = df[(df["discover_encoder"] == MAINLINE_DISCOVER_ENCODER) & (df["split_mode"] == "supervised") & (df["lm_backend"] == "sbert")]
...
def _discover_summary_tables(df: pd.DataFrame) -> dict[str, list[dict[str, object]]]:
    rows = []
    for encoder in ("magnn_legacy", "magnn", "han", "amdn_hage", "zeyan_coexpression"):
...
def _plot_detect_heatmap(df: pd.DataFrame, output_path: Path, *, metric: str, title: str) -> None:
    subset = df[(df["discover_encoder"] == MAINLINE_DISCOVER_ENCODER) & (df["lm_backend"] == "sbert") & (df["gnn_backend"] == "fusion_gnn")]
...
def _plot_discover_bar(df: pd.DataFrame, output_path: Path, *, metric: str, title: str) -> None:
    subset = df[df["encoder"].isin(["magnn_legacy", "magnn", "han", "amdn_hage", "zeyan_coexpression"])].copy()
...
def _build_markdown_report(
    ...
    lines = [
        "# CoordinationDiscover System Integration Report",
        "",
        "## System Status",
        ...
        "",
        f"## Detect Mainline ({MAINLINE_DISCOVER_ENCODER} + fusion_gnn + sbert)",
        "",
    ...
    lines.extend(
        [
            "",
            f"## Detect Backend Comparison (supervised, {MAINLINE_DISCOVER_ENCODER}, sbert)",
            "",
    ...
    lines.extend(
        [
            ...
            "## Deliverables",
            "",
            "- `coordination_discover_detect_mainline_table.csv`",
            "- `coordination_discover_detect_backend_comparison.csv`",
            "- `coordination_discover_discover_encoder_comparison.csv`",
            "- `coordination_discover_detect_auc_heatmap.png`",
            "- `coordination_discover_detect_macro_f1_heatmap.png`",
            "- `coordination_discover_discover_modularity_bar.png`",
        ]
    )
    return "\n".join(lines) + "\n"
