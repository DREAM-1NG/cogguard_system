"""Review main-system integration tests.

These tests lock the system-level contract before wiring Review into the main
...
def test_provider_key_encryption_masks_and_round_trips(monkeypatch):
    from app.services.review_system_service import decrypt_provider_api_key
    from app.services.review_system_service import encrypted_provider_payload
    from app.services.review_system_service import provider_public_view

    monkeypatch.setattr("app.services.review_system_service.settings.REVIEW_CONFIG_ENCRYPTION_KEY", "unit-test-secret")
...
def test_missing_encryption_key_fails_closed(monkeypatch):
    ...
    monkeypatch.setattr("app.services.review_system_service.settings.REVIEW_CONFIG_ENCRYPTION_KEY", "")

    with pytest.raises(ValueError) as exc_info:
        encrypted_provider_payload({"api_key": "sk-test"})

    assert "REVIEW_CONFIG_ENCRYPTION_KEY" in str(exc_info.value)


def test_env_retrieval_provider_public_view(monkeypatch):
    from app.services.review_system_service import env_provider_public_view

    monkeypatch.setattr("app.services.review_system_service.settings.REVIEW_RETRIEVAL_API_KEY", "exa-test-secret")
    monkeypatch.setattr("app.services.review_system_service.settings.REVIEW_RETRIEVAL_BASE_URL", "https://api.exa.ai")
    monkeypatch.setattr("app.services.review_system_service.settings.REVIEW_RETRIEVAL_SEARCH_PATH", "/search")
    monkeypatch.setattr("app.services.review_system_service.settings.REVIEW_RETRIEVAL_PROVIDER_NAME", "Exa")
    monkeypatch.setattr("app.services.review_system_service.settings.REVIEW_RETRIEVAL_ADAPTER", "exa")
...
def test_gate_dataset_payload_normalizes_cases_and_fingerprint():
    from app.services.review_system_service import normalize_gate_dataset_upload
...
def test_persist_agent_review_rows_preserves_natural_language_primary_output():
    from app.services.review_system_service import normalize_agent_review_result
...
def test_backfill_report_json_is_idempotent():
    from app.services.review_system_service import extract_review_backfill_records
    ...
    records = extract_review_backfill_records("risk-1", report_json, created_by=7)
...
def test_exa_retrieval_adapter_maps_request_and_response(monkeypatch):
    from app.tasks.review_tasks import _build_http_retrieval_provider
    import asyncio
    ...
    monkeypatch.setattr("app.tasks.review_tasks.httpx.AsyncClient", FakeAsyncClient)
