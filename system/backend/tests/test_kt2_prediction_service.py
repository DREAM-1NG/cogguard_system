...
def test_kt2_event_adapter_builds_bundle_from_internal_research_runtime():
    bundle = kt2_prediction_service.build_event_inference_bundle(
        _posts(),
        [],
        max_sequence_len=8,
...
def test_kt2_event_checkpoint_missing_uses_internal_adapter(tmp_path):
    result = kt2_prediction_service.predict_event_with_checkpoint(
...
def test_kt2_current_event_prediction_uses_internal_live_runtime():
    result = asyncio.run(
        kt2_prediction_service.predict_event_macro_micro(
            posts=_posts(),
            comments=[],
            top_k=2,
        )
    ...
    assert result["model"] == "KT2LiveRuntime"
...
def test_kt2_current_event_prediction_adds_hindcast_protocol():
    result = asyncio.run(
        kt2_prediction_service.predict_event_macro_micro(
            posts=_posts(),
            comments=[],
    ...
    assert result["schema"] == "cogguard.kt2.hindcast_protocol.v1"
    assert result["model_version"] == "kt2-hindcast-protocol-v1"
...
def test_kt2_public_dataset_loader_normalizes_jsonl_fixture(tmp_path):
    ...
    result = kt2_prediction_service.load_public_cascade_fixture(fixture, dataset="twitter")

    assert result["status"] == "ok"
    assert result["schema"] == "cogguard.kt2.public_cascade_fixture.v1"
