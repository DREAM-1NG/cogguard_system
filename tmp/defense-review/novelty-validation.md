...
> 独立核验者视角，2026-06-02。不盲信 kt1/kt2/kt3-findings 里的文献结论，全部用 WebSearch/WebFetch 重新抓 arXiv 摘要核对。
...
| 文献 | arXiv | 核验结果 | 摘要要点 |
|---|---|---|---|
| Luceri et al. *CIB on TikTok* | 2505.10867 | 【真实】2025-05，v2 2025-10 | 793K TikTok 2024 大选视频。**原文明确**："traditional coordination indicators generalize well to TikTok"，而 transcript 文本相似 + Duet/Stitch 失效。即"行为型协同信号可迁移、内容型不可迁移"是该文**已发表的实证结论**，不是 KT1 首创。 |
| Schneider, Yuan, Rizoiu *Beyond Content* | 2602.02838 | 【真实】2026-02-02，arXiv preprint，未过审 | "platform-agnostic framework that identifies malicious actors from their behavioral policies"，把用户活动建成序列决策过程。policy 分类器 macro-F1 94.9% vs 文本 91.2%，且"degrade more gracefully under evasion"。**这正是 KT1 想讲的 behavior-over-content + platform-agnostic + 抗规避，已被人系统化做出并量化。** |
...
## 二、KT1 协同检测 — 裁决：**增量偏被覆盖（最危险）**

### 最接近的先前工作（双料撞车）
1. **Schneider, Yuan, Rizoiu. *Beyond Content: Behavioral Policies Reveal Actors in Information Operations*. arXiv:2602.02838, 2026.** — 直接撞 KT1 的核心 thesis。
2. **Luceri et al. *Coordinated Inauthentic Behavior on TikTok*. arXiv:2505.10867, 2025.** — 直接撞 KT1 的"行为型信号比内容型更可迁移"论证。
...
### 精确 delta
- KT1 的两条主张本身——"behavior-over-content"与"platform-agnostic 行为信号更可迁移/抗规避"——**已被 2602.02838 与 2505.10867 分别明确做出并量化**。KT1 不是首次提出这个命题，最多是**复述既有结论**。
- KT1 真实落地代码只是 CooRTweet 共享对象配对 + 百分位阈值 + 社区发现（findings 已证 `detector.py` / `network.py`）；声称的 PSL（对称超几何 + Cauchy + pair-level BH-FDR）+ Semantic Adapter + 多通道融合**0 行落地**。
...
### 裁决
- "behavior-over-content / platform-agnostic"作为**主创新主张 = 被覆盖**，绝不能作为 KT1 的卖点首发权来讲，2602.02838 会被评委一句话击穿。
...
## 三、KT2 传播监控 — 裁决：规模预测部分**可辩护（窄）**；传播路径预测**不成立（无设计无代码）**
...
### 精确 delta
- KT2 的 CascadeSwitch = **零训练 + 4 体制 regime-switching（Hamilton-style）+ softmax 后验 + mixture forecast + LLM 仅做可解释事件抽取**（findings: `regime_model.py`/`trend_predictor.py` 已落地）。
...
- **被夸大的主张**：若对外材料写"传播路径预测"作为关键技术，**直接证伪**，必须立即改口径为"传播路径重建/关键路径取证"或列入未来工作。这是 KT2 最危险的表述风险。
...
## 四、KT3 报告研判 — 裁决：硬算法核**可辩护（窄白盒贡献）**；多 Agent+RAG 外壳**被覆盖**

### 最接近的先前工作
- **Tseng et al. *An Agentic Operationalization of DISARM for FIMI*. arXiv:2601.15109, 2026 (ICMCIS).** — 撞 KT3 的"agent + DISARM + 可审计推理"外壳。
- **Gautam. *Multi-agent Systems for Misinformation Lifecycle*. arXiv:2505.17511, 2025.** — 撞 KT3 的"多 agent + 全生命周期 + 证据/溯源 + 可解释"。
- **Avram et al. arXiv:2508.10143, 2025**（MCP 多 agent 虚假信息检测）。
- 关键独立发现（findings 未点的更危险撞车，在**算法核**层面）：
  - **MITRE TIE (Technique Inference Engine), CTID 2024** + **Markov-chain attack-chain prediction thesis (UMassD)** + **arXiv:2508.18230 (2025, directed-graph inter-phase ATT&CK technique prediction)** — 这些已在 **ATT&CK 域**实现了"**技术转换概率图 → 预测下一步技术 → 多步路径生成**"，正是 KT3 `disarm_scorer.py`（TRANSITIONS + TRANSITION_PROBS + predict_next_techniques）的算法原语。

### 精确 delta
- KT3 真落地的硬核（findings 已证 `phase_detector.py` / `disarm_scorer.py` / `ds_fusion.py` 约 1340 行，CPU 白盒）：**5 阶段生命周期 + logistic hazard breakout 预测 + DISARM 转换图预测下一技术 + 阶段调制 D-S 融合**。
- 相对 2601.15109 的 delta **成立且重要**：2601.15109 是 flat tagging（观测→DISARM 标签），**摘要确无 next-technique/path 预测**；KT3 做转换图 + 下一步预测 + 据预测生成反制。这是真区别。
- **但**："技术转换概率图 + 预测下一步技术 + 多步路径评分"这一算法范式在 **ATT&CK/CTI 域已是成熟做法**（MITRE TIE / Markov attack-chain / 2508.18230）。KT3 的真 delta 因此**收窄为"把该范式迁移到 DISARM 影响行动域 + 叠加 phase-conditioned hazard + D-S 冲突融合"**，而非"白盒路径预测"本身的原创。
- Phase-Aware Hazard：rumor/cascade 的多阶段 + 早期预警/breakout 预测在文献中已有（多 stage SIR、early-warning signals arXiv:2505.24795，早期扩散信号 UChicago）。KT3 的 hazard 是 domain-informed 手工权重 logistic，**delta 在工程可审计性而非建模新颖性**。
...
| 模块 | 主张 | 裁决 | 最危险撞车 |
|---|---|---|---|
| KT1 | behavior-over-content / platform-agnostic 协同 | **被覆盖** | Schneider/Rizoiu 2602.02838 |
| KT1 | PSL pair-level 显著性框架（若落地） | 增量→可辩护 | 无直接撞车（但 0 行代码） |
| KT2 | CascadeSwitch 零训练 regime-switching 规模预测 | **窄可辩护** | CasFT 2409.16619 / AutoCas 2502.18040 |
| KT2 | 传播路径预测 | **不成立（无设计无代码）** | 概念上被任意 next-hop 预测工作压制 |
| KT3 | 多 Agent+RAG 报告研判 | **被覆盖** | Gautam 2505.17511 / 2601.15109 |
| KT3 | Phase-Aware Hazard + DISARM 路径预判（白盒 CPU） | **窄可辩护** | MITRE TIE / Markov attack-chain / 2508.18230（算法原语已存在于 ATT&CK 域） |
