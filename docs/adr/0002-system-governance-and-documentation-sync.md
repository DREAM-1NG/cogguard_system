...
docs still contained stale paths such as `new-system/`, and new KT3 Student work
...
KT3 Teacher Silver and Selective Student logic are split out of
`kt3_trainable_post.py` into focused modules:

- `system/backend/app/core/risk/kt3_teacher_silver.py`
- `system/backend/app/core/risk/kt3_selective_student.py`

`kt3_trainable_post.py` remains the shared post-feature and legacy view
...
- Keeping all KT3 Student and Teacher Silver logic inside `kt3_trainable_post.py`:
