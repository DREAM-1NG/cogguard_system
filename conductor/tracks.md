...
| Claim response landscape | Local validation complete; live check pending Docker | Exact authority-account bindings and a path-backed view of official publications and influential responses | Run the authority-account migration and verify the authenticated browser flow against real event data once Docker services are available; do not infer official identity from names or platform verification. |
