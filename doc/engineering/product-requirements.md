...
CogGuard 面向网络舆论对抗场景，以 **跨平台协同攻击行为** 为核心监测对象，围绕"协同发现 - 传播监控 - 风险评判"形成三阶段分析闭环。

**核心问题对象**：跨平台协同操纵网络——研究对象从单账号/账号群体转为跨平台协同操纵网络，主流程从账号分类转为证据驱动的发现、归因与风险研判闭环。
...
**功能三：风险评判** — 针对攻击进行分析并生成报告，显式利用前两个功能所生成的结果。
...
|---|----------|----------|-----------|-------------|
| KT1 | 基于多任务框架的跨平台协同检测 | 协同发现 | `@ARIS:pending` | 输入：行为对集合 → 输出：协同判定 + 类型标签 + 群组 + 证据边 |
| KT2 | 基于 LLM 增强的传播趋势预测 | 传播监控 | `@ARIS:pending` | 输入：传播时序数据 + 事件上下文 → 输出：未来 N 小时趋势预测 |
| KT3 | 基于 Agent+RAG 的攻击分析与报告生成 | 风险评判 | `@ARIS:pending` | 输入：KT1 结果 + KT2 结果 + 知识库 → 输出：结构化报告 + DISARM 映射 + 处置建议 |
...
| 风险评判 | Risk Assessment | 系统第三阶段功能，针对攻击进行分析并生成可审计报告 |
...
5. 点击「开始采集」按钮，系统创建 Celery 异步任务
6. 在"采集任务列表"中查看任务状态（pending → running → done/failed）
7. 任务完成后，在"采集数据"表格中浏览已采集的帖子/评论数据
8. 如需取消运行中的任务，点击「取消」；如需删除任务及其数据，点击「删除」

**预期结果**：
- 采集任务创建成功，状态流转正常
- 帖子数据存入 MongoDB，可在采集数据表格中分页浏览
- 数据包含：平台、作者、内容、发布时间、互动指标等字段
...
2. 配置检测参数：
   ...
   - 可选限定平台
3. 点击「运行检测」，系统基于 CooRTweet 算法执行协同检测
4. 查看概览统计：分析帖子数、协调配对数、协调账户数、群体数量
5. 在"协同网络"可视化区域查看协同网络图（Canvas 力导向布局）
6. 在"协调账户排名"表格中查看各账户的连接数、平均边权、平均时间差、对称性等指标
7. 在"高频共享对象"表格中查看被协调分享的 URL/标签
...
5. 在"高频共享对象"表格中查看传播最广的 URL/标签及其分享次数、涉及账户数
6. 在"传播时间线"中按时间顺序查看帖子发布序列，协调账户以红色标记
7. 【待开发 @ARIS:KT2】点击「趋势预测」，调用 CascadeSwitch 模型预测未来 N 小时传播走势
...
**涉及页面**：监测看板（`/`）
**涉及 API**：【待开发】`GET /api/v1/dashboard/stats`、`GET /api/v1/dashboard/trends`

### 2.7 场景 S6：账户画像深度分析

**背景**：分析人员需要对采集数据中的账户进行行为画像分析，识别高自动化倾向的可疑账户，为协同发现和传播归因提供先验支撑。
...
| US-303 | analyst | 作为分析人员，我希望预测传播趋势，以便提前预判事件发展走向 | 【@ARIS:KT2】输出未来 N 小时传播量/范围/速度预测值 | P0 | `/propagation` |
...
本章定义 CogGuard 系统的前端页面信息架构、各页面功能区域划分、关联 API 以及页面间跳转关系。系统采用 Vue 3 + Ant Design Vue + Vue Router 构建单页应用，通过 BasicLayout 提供统一的侧边栏导航 + 顶部栏 + 标签页 + 内容区布局。当前已完成登录、数据采集、协同检测、传播归因、账户监测等核心页面，监测看板与报告管理为待开发模块。
...
| `/propagation` | 传播归因 | ShareAltOutlined | 分析信息传播路径，识别起爆/桥接/扩散关键角色 |
| `/accounts` | 账户监测 | UserOutlined | 账户行为画像、作息节律、自动化倾向评估 |
| `/risk` | 风险研判 | AlertOutlined | 阶段感知风险评估、D-S 证据融合、DISARM 攻击路径分析 |
...
系统首页，提供全局数据概览。当前为占位实现（统计卡片值均为 0，图表区域为空状态），需接入后端聚合接口实现实时数据展示。
...
| 统计卡片行 | 4 个 Statistic 卡片，等宽排列（span=6） | 占位，需接入 API |
| ECharts 图表区 | 趋势图 + 分布图，2 列布局 | 待开发 |
| 最近预警列表 | 最新风险预警事件列表 | 待开发 |

### 详细设计：统计卡片

| 卡片 | 数据字段 | 后缀 | 说明 |
|------|----------|------|------|
| 监测任务 | `active_jobs` | 个 | 当前运行中的采集任务数 |
| 采集数据 | `total_posts` | 条 | 已采集帖子/评论总量 |
| 风险预警 | `risk_alerts` | 条 | 近 24h 内触发的风险预警数 |
| 协同群体 | `coord_groups` | 个 | 最近一次协同检测发现的群体数 |
...
- X 轴：近 7 天日期
- Y 轴：每日新增帖子数
- 数据源：`GET /api/v1/dashboard/overview` 返回的 `daily_posts` 数组
- 交互：鼠标悬停显示 Tooltip，支持缩放

**右侧 — 风险等级分布饼图（span=12）**：
- 分类：critical（红）、high（橙）、medium（黄）、low（绿）
- 数据源：`GET /api/v1/dashboard/overview` 返回的 `risk_distribution` 对象
- 交互：点击扇区可跳转至风险研判页面并筛选对应等级

### 详细设计：最近预警列表

- 展示最近 10 条风险预警事件
- 列定义：时间、平台、风险等级（Tag 颜色编码）、摘要、操作（查看详情）
- 点击「查看详情」跳转至 `/risk` 页面对应报告
- 数据源：`GET /api/v1/dashboard/overview` 返回的 `recent_alerts` 数组

### 需新增 API

| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/api/v1/dashboard/overview` | 聚合看板数据：统计卡片、趋势图、分布图、预警列表 |
...
```json
{
  ...
  "data": {
    ...
    "coord_groups": 4,
    "daily_posts": [
      { "date": "2026-04-23", "count": 1820 },
      { "date": "2026-04-24", "count": 2105 }
    ],
    "risk_distribution": {
      "critical": 2, "high": 5, "medium": 12, "low": 28
    },
    "recent_alerts": [
      {
        "id": "rpt_xxx",
        "platform": "weibo",
        "risk_level": "high",
        "summary": "检测到 12 个账户协同传播虚假信息",
        "assessed_at": "2026-04-29T10:30:00"
      }
    ]
  }
}
...
> **源文件**：`G:\CISCN\cogguard_system\new-system\frontend\src\views\dashboard\index.vue`（当前占位）
...
跨平台数据采集管理页面，支持创建采集任务、查看任务列表与状态、浏览已采集数据。支持 Mock 微博、微博（MediaCrawler）、抖音、小红书、新闻链接等多平台。
...
| 创建采集任务 | 内联表单：平台选择、关键词输入、链接输入（textarea）、最大帖子数、开始采集按钮 |
...
| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/api/v1/coordination/detect` | 运行协同检测，参数：time_window、min_participation、edge_weight、platform |
...
## 3.6 传播归因（/propagation）— 已完成

### 功能概述

基于共享对象的时序关系构建传播子图，识别信息传播链中的三类关键角色：起爆节点（最早发布者）、桥接节点（连接不同群体，介数中心性高）、扩散节点（被大量跟随传播）。

### 功能区域划分

| 区域 | 说明 |
|------|------|
| 页面头部 | PageHeader：标题 "传播归因" + 三类角色说明 |
...
| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/api/v1/propagation/analyze` | 运行传播归因分析，可选参数：platform |
| POST | `/api/v1/propagation/predict-trend` | 预测传播趋势（CascadeSwitch），可选参数：platform |
...
| 区域 | 说明 | 状态 |
|------|------|------|
| 页面头部 | PageHeader：标题 "风险研判" + 方法论描述 | 已完成 |
...
```
登录 → 风险研判 → 选择平台/参数 → 运行评估 → 查看结果 → 导出报告
```

**路径 3：日常监控**

```
登录 → 看板概览 → 查看预警列表 → 点击预警进入风险研判 → 查看详情
```
...
CogGuard 系统围绕"数据采集 → 协同发现 → 传播监控 → 风险评判"四阶段流水线构建。本章定义三大核心功能（协同发现、传播监控、风险评判）的数据流转关系、触发条件、子功能清单及优先级分期，并以代码实现为基准标注各子功能的完成状态。所有标注为"待 ARIS"的关键技术子功能，将在 ARIS 工作空间中完成算法研发后回填至主系统。
...
| **风险评判** | `coord_data`, `prop_data`, `acct_data` | 风险报告 | `report: {report_id, event_id, platform, scores: {overall_risk_score, risk_level, manipulation, authenticity, impact}, phase, fusion, disarm_analysis, risk_factors, recommendations}` |
...
| 风险评判 | 手动触发 | 用户调用风险评估 API | 自动编排上游三个服务，无需手动前置调用 |
| 风险评判（自动） | P2 待开发 | 预警系统触发 | 基于阈值或定时任务自动执行 |

### 4.2.2 依赖矩阵

| 功能 (行依赖列) | 数据采集 | 协同发现 | 传播监控 | 趋势预测 | 账户画像 | 风险评判 |
|-----------------|:--------:|:--------:|:--------:|:--------:|:--------:|:--------:|
| **数据采集**     | —        |          |          |          |          |          |
| **协同发现**     | **必需** | —        |          |          |          |          |
| **传播监控**     | **必需** |          | —        |          |          |          |
...
| **状态** | 待 ARIS 工作空间实现 |
| **实现文件** | 待创建 |
| **描述** | 基于多任务学习框架的协同行为检测。在共享对象时间窗检测的基础上，引入内容语义相似度、行为序列模式等多维特征，通过多任务神经网络联合学习协同概率。该子功能为竞赛关键技术 KT1 的核心增强。 |
...
风险评判模块是系统的最终决策层，编排上游三大模块的输出，通过证据构建 → 阶段检测 → D-S 融合 → DISARM 评分的流水线生成结构化风险报告。核心实现位于 `app/core/risk/`。
...
| **状态** | 待 ARIS 工作空间实现 |
| **实现文件** | 待创建 |
| **描述** | 基于 LLM Agent 架构 + RAG 检索增强，对 DISARM 攻击路径进行深度分析。Agent 自主检索历史案例库和 DISARM 知识库，结合当前证据包生成攻击意图推理、战术演进预测和定制化反制方案。该子功能为竞赛关键技术 KT3 的核心增强。 |
...
| 多任务框架协同检测 | 协同发现 | **待 ARIS 工作空间实现** | 关键技术 KT1 |
| LLM 增强趋势预测 | 传播监控 | **待 ARIS 工作空间实现** | 关键技术 KT2（框架已就绪，需配置 API） |
| Agent + RAG 攻击分析 | 风险评判 | **待 ARIS 工作空间实现** | 关键技术 KT3 |
| LLM 桥接 | 风险评判 | 占位 | 接口已定义，待 LLM API 启用 |
...
| | LLM 增强趋势预测 | P1 | 待 ARIS | `core/propagation/llm_context.py` | KT2 |
| | 立场检测 | P2 | 待开发 | 待创建 | |
| | 危害性评估 | P2 | 待开发 | 待创建 | |
| | 源头追溯 | P2 | 待开发 | 待创建 | |
| **风险评判** | 证据构建 | P0 | 已完成 | `core/risk/evidence_builder.py` | |
...
| `/api/v1/propagation/analyze` | GET | Y | Y | Y | 传播归因分析 |
...
## 5.5 传播归因与趋势预测模块 API（/api/v1/propagation/*）
...
| **API 路由** | `api/v1/propagation.py` | 已实现 | 传播归因接口 |
| **API 路由** | `api/v1/accounts.py` | 已实现 | 账户监测接口 |
| **API 路由** | `api/v1/risk.py` | 已实现 | 风险研判接口 |
...
| 影响力 | `impact` | 传播归因模块 | 桥接比例、突发性、跨集群传播 |
