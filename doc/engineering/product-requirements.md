...
| KT3 | 基于 Agent+RAG 的攻击分析与报告生成 | 报告研判 | `@ARIS:pending` | 输入：KT1 结果 + KT2 结果 + 知识库 → 输出：结构化报告 + DISARM 映射 + 处置建议 |
...
4. 系统基于 CooRTweet/后续 KT1 算法执行协同检测，并输出用户聚类结果
...
8. 【待开发 @ARIS:KT2】点击「趋势预测」，调用 CascadeSwitch 模型预测未来 N 小时传播走势
...
| Agent + RAG 攻击分析 | 报告研判 | **待 ARIS 工作空间实现** | 关键技术 KT3 |
