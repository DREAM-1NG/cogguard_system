...
文档要求 KT1 从行为对集合输出协同判定、类型标签、群组和证据边。路线图显示共享对象 CooRTweet MVP 已完成，但多行为边构建和显著性筛查仍需加强。
...
文档要求 KT2 在传播子图和关键角色基础上输出趋势预测，并补齐立场检测、危害性评估等信号供 KT3 消费。路线图显示传播子图、时间线、关键角色已完成，WP4-5 立场/危害尚未启动。
...
文档要求 KT3 消费 KT1、KT2 与知识库，输出结构化报告、DISARM 映射和处置建议。路线图显示报告研判 MVP 已完成，但 LLM bridge、Agent+RAG 深度分析，以及预警/报告中心/处置跟踪仍待补齐。
...
5. 闭环跑通后，再分模块增强 KT1 多行为协同、KT2 立场/危害与趋势预测、KT3 报告中心和预警处置。
