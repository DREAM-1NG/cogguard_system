# KT3 PropagationTreeAgent 方法与开发计划
...
- 离线 Agent runner 将 `selected_tree_ids` 和 `kt3-propagation-context-v1` 传给 `PropagationTreeAgent`。
- 只有真实线程上下文或图边时才推荐 `PropagationTreeAgent`。
- 测试覆盖 thread 构建、上下文压缩和 claim-only 不触发传播树 Agent。

## 文献证据

| 方向 | 代表证据 | 可迁移方法 | KT3 采纳方式 |
| --- | --- | --- | --- |
...
- Agent context 优先读取 `kt3-propagation-context-v1`。
