...
| 是否被多功能消费 | ✅ 是 — KT1 用 bot_prob 过滤、KT2 用 stance 分布、KT3 用 KOL 标识 |
| 与上游 KT 关系 | 平行 + 支撑（非依赖） |
...
| F-ACCT-2 | 账号级 stance 聚合 | 聚合多帖立场为账号级分布 | F-PROP WP4 帖级 stance | `stance.distribution / extremity / consistency` | `core/account/stance_aggregator.py`（新增） | ❌ 依赖 KT2 WP4 |
...
### 3.2 F-ACCT-2 账号级 Stance 聚合（与 KT2 WP4 分层契约）
...
请求（profile_url 或 platform+account_id）
  ...
  │     对该用户的全部发文调用：
  │       • 风险检测   ← 复用 F-RISK harm_assessor 或 KT2 WP5（按帖级聚合）
...
homepage_response = {
    ...
    "content_checks": [  # 与 posts 一一对应
        {
            "post_id": ...,
            "harm": {"score": float, "dimensions": {...}},   # 复用 KT2 WP5
            "stance": {"label": str, "confidence": float},   # 复用 KT2 WP4
...
| `core/account/content_checker.py` | 3-4 h | engineering（编排 KT2 WP4/WP5 + F-COORD 模板检测的批处理） |
...
| P1 | E12 | F-ACCT-6 内容巡检：`content_checker.py` 编排 KT2 WP4/WP5 + 模板检测批处理 | 3-4 h |
...
- E9 阻塞于 **F-PROP WP4 stance_detector.py**（KT2 未启动）
...
- E12 阻塞于 **KT2 WP4 / WP5 / F-COORD 通道模板检测**——若上游未完成，content_checker 启用 mock fallback（仅给规则化结果）
...
2. **Stance 拆层：F-PROP=帖级 / F-ACCT=账号级聚合**
   - 理由：避免双轨实现，复用 KT2 WP4 算法成果
   - 影响：F-ACCT-2 阻塞于 KT2 WP4 完成
   - 替代方案：先在 F-ACCT 内做"轻量句级 stance"作为 fallback（如果 KT2 WP4 进度落后），但仍以 KT2 输出为最终来源
...
7. **F-ACCT-6 内容巡检全部复用上游算法（不重新实现）**
   - 理由：风险检测复用 KT2 WP5 / 立场复用 KT2 WP4 / 模板化复用 F-COORD 通道
   - 影响：F-ACCT-6 仅做编排 + 批处理，不涉及新算法 → research = 0
   - 降级：若上游未完成（特别是 KT2 WP4/5），content_checker 用 mock fallback（仅给规则化结果），并在响应中明确 `data_completeness="partial"`
...
for post in account_posts:
    harm = harm_assessor.score(post.content, post.metadata)        # 复用 KT2 WP5
    stance = stance_detector.detect(post.content)                  # 复用 KT2 WP4
...
- KT2 WP5 `harm_assessor.score(text, metadata) -> {score, dimensions}` 必须暴露为可单帖调用的纯函数
- KT2 WP4 `stance_detector.detect(text) -> {label, confidence}` 同样
