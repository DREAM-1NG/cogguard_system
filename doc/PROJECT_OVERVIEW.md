...
| 关键技术 | Coordination Discover / Detect 跨平台共同行为特征复用融合检测 ｜ Propagation Analysis LLM+时序的事件规模预测 ｜ Risk Review Phase-Aware Hazard + DISARM 路径预判（多 Agent 编排） |

**一句话现状**：数据采集、协同检测 MVP、传播趋势预测核心、报告研判白盒引擎均已落地可运行；三大关键技术各自的"创新增强层"（Coordination Discover / Detect 的 PSL、Propagation Analysis 的真实 LLM 接入与路径预测、Risk Review 的 Agent/RAG 层）多为设计稿，且存在 LLM 客户端依赖缺失这一共性阻塞。
...
- **功能一 · 协同发现（Coordination Discover / Detect，核心关键技术）**：用平台无关的共同行为特征发现协同群体
- **功能二 · 传播监控**：预测事件规模与传播态势
- **功能三 · 报告研判**：消费上游证据，做阶段预警 + 攻击路径预判 + 反制 + 结构化报告
...
> ⚠️ **共性阻塞（影响三大关键技术的增强层）**：
> - **无任何 LLM 客户端库**（无 openai / dashscope / deepseek SDK）——Propagation Analysis 真实事件提取、Risk Review Agent 全靠未来补依赖 + httpx 直连
> - **无 scipy / statsmodels** —— Coordination Discover / Detect 的 PSL（超几何、BH-FDR）无法实现
> - **无 sentence-transformers / text2vec** —— Coordination Discover / Detect 语义通道无支撑（但新方向已决定排除内容信号，影响弱化）
...
- **Detection（发现"谁在协同"）** —— Coordination Discover / Detect 核心创新所在
- **Characterization（刻画"协同群体是什么样"）** —— 四维表征：Authenticity / Orchestration / Time-variance（行为/结构型，Coordination Discover / Detect）+ Harmfulness（内容型，归 Risk Review）
...
> **职责边界（2026-06-03 厘清）**：Propagation Analysis **只做传播动力学**（规模/形态/路径预测），**不做内容分析**。立场检测、危害性评估等内容理解任务**已全部移交 Risk Review 的 Characterization 层**，Propagation Analysis 不再承载，以消除与 Risk Review 的职责重叠。
...
- ~~立场检测 / 危害性评估~~ → **移交 Risk Review**（见 §6.1）
...
| ~~立场检测 / 危害性评估~~ | — | ➡️ **移交 Risk Review** |

API：`GET /propagation/analyze`、`POST /propagation/predict-trend`。

### 5.4 关键技术陈述（答辩口径）

> Propagation Analysis 提出**事件条件下的分层传播预测框架**：用 LLM 把帖子内容抽成外生事件，驱动透明的体制后验 p(z)，并以该后验同时条件化宏观规模、中观形态、微观下一跳预测。区别于商业产品（知微，仅事后分析无预测）与现有级联模型（仅用观测序列、无外生事件条件）。
...
| 模块 | 文件 | 状态 | 说明 |
|------|------|------|------|
| 账户监测 | `core/account_profiler.py` (163) | ✅ 已落地 | 行为画像 + 自动化倾向评分（0-100），供 Coordination Discover / Detect/Risk Review 取证 |
...
**设计稿（有方案无代码）**：Coordination Discover / Detect 的 PSL/多通道、Risk Review 的 Agent 编排/RAG 报告/Layer 3 Agent、Propagation Analysis 的立场/危害子功能。

**缺口（无设计无代码）**：Propagation Analysis 传播路径预测（未来结构）、预警中心、报告中心、评估脚本与公开数据集基准。

### 9.2 三条共性阻塞

1. **LLM 客户端依赖缺失** —— `pyproject.toml` 无任何 LLM SDK。阻塞 Propagation Analysis 真实事件提取（现 `mock_llm=True`）+ Risk Review Agent 层。需先定 DeepSeek 接入并补依赖。
2. **统计/算法依赖缺失** —— 无 scipy/statsmodels，阻塞 Coordination Discover / Detect PSL 落地。
...
- 加分项：Propagation Analysis 预测"传播会涨多大"、Risk Review 预测"攻击者下一步用什么手法"，两套预测互补不冲突
...
- 答辩风险与文献核验：`tmp/defense-review/{coordination-discover,propagation-analysis,risk-review}-findings.md`、`examiner-review.md`、`novelty-validation.md`
